...
class TrackingsSocketApi {
  TrackingsSocketApi(this.trackingsCollection);
    ...
    return webSocketHandler((WebSocketChannel socket) {
      socket.stream.listen((message) async {
        ...
        if (data['action'] == 'DELETE') {
          await trackingsCollection.deleteOne({
        ...
        if (data['action'] == 'UPDATE') {
          ...
          await trackingsCollection.replaceOne(
              filter, Track.fromJson(data['payload']).toJson());
        }

        final adminsJson = await trackingsCollection.find().toList();
